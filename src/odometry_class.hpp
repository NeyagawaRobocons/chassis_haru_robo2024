...
private:
    ...
    pose2D pose;                    // 姿勢
    double tire_angle[3] = {0.0};        // エンコーダの角変位
...
public:
    ...
    // メンバ関数
    deltaPose2D calc_vel(double _tire_angle[3]);        // 微小変位を計算する関数
    void calc_pose(deltaPose2D delta_pose);   // 位置の積分
    void update_angles(double _tire_angle[3]);         // カウントの更新
    void update_pose(double _tire_angle[3]);           // 位置の更新
